defmodule ElixirTodo.TodosTest do
  ...
  describe "items" do
    ...
    test "create_item/1 with valid data creates a item" do
      valid_attrs = %{completed_at: ~U[2022-03-10 13:07:00Z], content: "some content"}

      assert {:ok, %Item{} = item} = Todos.create_item(valid_attrs)
      assert item.completed_at == ~U[2022-03-10 13:07:00Z]
    ...
    test "update_item/2 with valid data updates the item" do
      item = item_fixture()
      update_attrs = %{completed_at: ~U[2022-03-11 13:07:00Z], content: "some updated content"}

      assert {:ok, %Item{} = item} = Todos.update_item(item, update_attrs)
      assert item.completed_at == ~U[2022-03-11 13:07:00Z]
